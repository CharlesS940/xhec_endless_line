...
def load_attractions_location():
    return pd.read_csv('data/link_attraction_park.csv', sep=';')  # which rides are in which park
...
class ThemeParkGraph:
    ...
    def visualize(self, use_distance_layout=True, figsize=(20, 20)):
        ...
        plt.close(fig)  # Close the plot to avoid duplication when displaying
        
        # Return the plot and the pos_scaled layout
...
PortAventura_attractions = list(link_attractions_df[link_attractions_df['PARK'] == 'PortAventura World']['ATTRACTION'])
...
preferences = st.sidebar.multiselect(
    ...
    default=PortAventura_attractions[:3],
...
date_of_visit = st.sidebar.date_input(
    "Date of Visit",
    datetime.datetime(2018, 1, 1),  # Default value: 1st January 2018
...
fig, pos_scaled = PortAventura_park.visualize(use_distance_layout=True)
...
def evaluate_itinerary(itinerary, start_time, waiting_df, date, overlapping_visitors, theme_park, user_preferences, end_time):
    ...
    for i, attraction in enumerate(itinerary):
        ...
        current_time += (predicted_wait_time + ride_duration + math.ceil(travel_time)) / 60
    
    # Fill remaining time with low-wait attractions
    schedule = fill_gaps(schedule, current_time, end_time, waiting_df, theme_park)
    
    score = compute_score(total_wait_time, total_distance, total_crowd_penalty, itinerary, user_preferences)
...
def fill_gaps(schedule, current_time, end_time, waiting_df, theme_park):
    ...
    if available_time > 0.25 or available_time < -0.25:
        low_wait_attractions = get_low_wait_time_attractions(waiting_df, current_time, available_time)
        
        for attraction in low_wait_attractions:
            wait_time = predict_wait_time(attraction, current_time, waiting_df)
            ride_duration = get_ride_duration(attraction, waiting_df)
            travel_time = get_travel_time(schedule[-1]['attraction'], attraction, theme_park) if schedule else 0
...
def get_low_wait_time_attractions(waiting_df, current_time, available_time):
    filtered = waiting_df[(waiting_df["DEB_TIME_HOUR"] >= current_time) & (waiting_df["WAIT_TIME_MAX"] < 10)]
...
def optimize_schedule(new_visitor, synthetic_users, theme_park, waiting_df, date):
    ...
    for itinerary in attraction_permutations:
        schedule, score = evaluate_itinerary(
            itinerary, new_visitor["entry_time"], waiting_df, date, overlapping_visitors, theme_park, new_visitor["preferences"], new_visitor["exit_time"]
        )
...
# Run optimization
# Using inputs
optimized_schedule = optimize_schedule(new_visitor, synthetic_users, PortAventura_park, waiting_df, date_of_visit)
...
sampled_visitors = synthetic_users.sample(n=500, replace=True).reset_index(drop=True)

sampled_visitors_dict = sampled_visitors.to_dict(orient="records")

capacity_dict = dict(zip(ride_capacity['ENTITY_DESCRIPTION_SHORT'], ride_capacity['CAPACITY']))
...
for time in time_intervals:
    ...
    active_visitors = schedule_df[(schedule_df["arrival_time"] <= time) & (schedule_df["departure_time"] >= time)]
